...
=============================================
Streams readings to the deployed Render server over WebSocket.
...
# └─────────────────────────────────────────────────────────────────────────┘
SERVER_WS = "ws://127.0.0.1:8000/ws/sensor"
# For production Render deployment, use: "wss://your-app-name.onrender.com/ws/sensor"
# ── Model download ────────────────────────────────────────────────────────────
...
EAR_CLOSED = 0.15;  EAR_OPEN = 0.32

# ── Scoring ───────────────────────────────────────────────────────────────────
W_PERCLOS = 0.40;  W_TURN = 0.35;  W_TILT = 0.25
TURN_SAFE = 30.0;  TURN_MAX = 75.0
TILT_SAFE =  10.0;  TILT_MAX = 35.0
PERCLOS_WINDOW       = 50
EYE_CLOSED_THRESHOLD = 30.0
FLASH_COOLDOWN       = {3: 2.0, 2: 5.0, 1: 8.0}
PUSH_INTERVAL        = 0.1   # seconds between server pushes
...
C_WHITE  = (230, 230, 230);  C_GRAY   = (110, 110, 110)
C_GREEN  = ( 80, 200, 120);  C_AMBER  = ( 40, 190, 240)
C_ORANGE = ( 30, 140, 255);  C_RED    = ( 60,  60, 220)
C_PANEL  = ( 22,  22,  22);  C_BAR_BG = ( 55,  55,  55)
...
def eye_aspect_ratio(lm, indices, w, h):
    ...
    return (v1 + v2) / (2.0 * np.linalg.norm(pts[0] - pts[3]) + 1e-6)
...
def head_pose(lm, w, h):
    ...
    ok, rvec, _ = cv2.solvePnP(MODEL_POINTS, img_pts, cam_mat,
...
def perclos_score(p): return float(np.clip(100 - p, 0, 100))

def turn_score(yaw):
    a = abs(yaw)
    if a <= TURN_SAFE: return 100.0
    if a >= TURN_MAX:  return 0.0
    return float(100 - (a - TURN_SAFE) / (TURN_MAX - TURN_SAFE) * 100)

def tilt_score(roll):
    a = abs(roll)
    if a <= TILT_SAFE: return 100.0
    if a >= TILT_MAX:  return 0.0
    return float(100 - (a - TILT_SAFE) / (TILT_MAX - TILT_SAFE) * 100)

def compute_awareness(perclos_pct, yaw, roll):
    s_p = perclos_score(perclos_pct)
    s_t = turn_score(yaw)
    s_r = tilt_score(roll)
    score = s_p * W_PERCLOS + s_t * W_TURN + s_r * W_TILT
    if s_p < 50: score = 0
    elif s_t < 30: score = 0
    subs = {"perclos": s_p, "turn": s_t, "tilt": s_r}
    return round(score, 1), min(subs, key=subs.get), s_p, s_t, s_r
...
# ── Shared state (sent to server via WebSocket) ──────────────────────────────
latest = {
    "score": 100.0,
    "alert_level": 0,
    "message": "",
...
ws_status = {"connected": False}   # shared flag for UI dot
    ...
    while True:
        try:
            async with websockets.connect(
                ...
                print(f"[ws] Connected → {SERVER_WS}", flush=True)
                while True:
                    try:
                        payload = send_queue.get(timeout=0.05)
                        await ws.send(json.dumps(payload))
                    except queue.Empty:
                        await asyncio.sleep(0.01)
        except Exception as e:
            ws_status["connected"] = False
            print(f"[ws] Disconnected ({e.__class__.__name__}), retrying in 3s…", flush=True)
...
def push(payload: dict):
    """Send payload to server via WebSocket"""
    try:
...
def factor_bar(panel, label, sub_score, x, y, bw=170):
    color = (C_GREEN if sub_score >= 70 else C_AMBER if sub_score >= 50
             else C_ORANGE if sub_score >= 30 else C_RED)
    cv2.rectangle(panel, (x, y), (x + bw, y + 7), C_BAR_BG, -1)
    fill = int(bw * sub_score / 100)
    if fill > 0:
        cv2.rectangle(panel, (x, y), (x + fill, y + 7), color, -1)
    cv2.putText(panel, f"{label}  {sub_score:.0f}",
                (x, y - 7), FONT, 0.42, C_WHITE, 1, cv2.LINE_AA)

def draw_panel(canvas, w, h, l_pct, r_pct, perclos_pct, yaw, roll,
               score, dominant, s_perclos, s_turn, s_tilt, alert_lvl, do_flash):
    PANEL_H = 210
    ...
    cv2.putText(panel, f"Left eye   : {l_pct:.0f}%",             (lx, 28),  FONT, 0.45, C_WHITE, 1, cv2.LINE_AA)
    cv2.putText(panel, f"Right eye  : {r_pct:.0f}%",             (lx, 52),  FONT, 0.45, C_WHITE, 1, cv2.LINE_AA)
    cv2.putText(panel, f"PERCLOS    : {perclos_pct:.0f}% closed", (lx, 76),  FONT, 0.45, C_AMBER, 1, cv2.LINE_AA)
    tilt_dir = "L" if roll < -2 else ("R" if roll >  2 else "-")
    turn_dir = "L" if yaw  >  2 else ("R" if yaw  < -2 else "-")
    cv2.putText(panel, f"Head tilt  : {abs(roll):.1f} deg {tilt_dir}", (lx, 100), FONT, 0.45, C_WHITE, 1, cv2.LINE_AA)
    cv2.putText(panel, f"Head turn  : {abs(yaw):.1f} deg {turn_dir}",  (lx, 124), FONT, 0.45, C_WHITE, 1, cv2.LINE_AA)

    # Divider
    mid = w // 2
    cv2.line(panel, (mid - 10, 10), (mid - 10, PANEL_H - 10), (50,50,50), 1)

    # Right column — factor bars
    rx = mid + 10
    ...
    # Alert message — always visible while condition active
    if alert_lvl > 0:
...
def main():
    ...
    print("Driver Awareness Monitor — press Q to quit")
    print(f"Pushing to → {SERVER_WS}")
    print()
    ...
    last_flash_t = {1: 0.0, 2: 0.0, 3: 0.0}
    last_push_t  = 0.0
    ...
    while True:
        ...
        perclos_pct = (sum(1 for e in eye_history if e < EYE_CLOSED_THRESHOLD)
                        / len(eye_history)) * 100.0
        ...
        # Push to server (throttled to PUSH_INTERVAL)
        now = time.time()
        if now - last_push_t >= PUSH_INTERVAL:
            last_push_t = now
            msg_text = MESSAGES.get((dominant, lvl), ("", None))[0] if lvl > 0 else ""
            
            # Update global latest dict and send to server
            global latest
            latest.update({
                "score": round(score, 1),
                "alert_level": lvl,
                "message": msg_text,
                "dominant": dominant,
                "s_perclos": round(s_perclos, 1),
                "s_turn": round(s_turn, 1),
                "s_tilt": round(s_tilt, 1),
                "perclos_pct": round(perclos_pct, 1),
                "yaw": round(yaw, 1),
                "roll": round(roll, 1),
                "timestamp": now,
                "sensor_connected": ws_status["connected"],
            })
            
            # Send entire latest dict to server via WebSocket
            push(latest.copy())
                
        # Flash cooldown (border only)
        now      = time.time()
        lvl      = last["alert_lvl"]
        do_flash = False
