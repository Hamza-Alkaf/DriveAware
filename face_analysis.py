...
TURN_SAFE = 35.0;  TURN_MAX = 50.0
TILT_SAFE =  8.0;  TILT_MAX = 25.0
PERCLOS_WINDOW       = 50
...
def draw_panel(canvas, w, h, l_pct, r_pct, perclos_pct, yaw, roll,
    ...
    # WebSocket status indicator (top-right corner of panel)
    dot_col = C_GREEN if ws_status["connected"] else (60, 60, 60)
    dot_lbl = "LIVE" if ws_status["connected"] else "OFF"
    cv2.circle(panel, (w - 20, 15), 5, dot_col, -1)
    cv2.putText(panel, dot_lbl, (w - 55, 19), FONT, 0.35, dot_col, 1, cv2.LINE_AA)
